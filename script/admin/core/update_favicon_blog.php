...
$destn_file = 'icon'.$user_id.''.get_rand_numbers(3).'.'.$imageFileType.'';
$destn_upload = $target_dir . $destn_file;

if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "ico") {
$_SESSION['reply'] = array (array("warning","Only JPG, PNG, JPEG and ICO files are allowed"));
...
$stmt = $conn->prepare("UPDATE tbl_web_info SET favicon = ?");
$stmt->execute([$destn_file]);
$_SESSION['reply'] = array (array("success","Favicon updated successfully"));
header("location:../blog-settings");
