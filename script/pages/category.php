...
if (isset($_GET['key'])) {
  ...
  if (count($result) < 1) {
    ...
    foreach($result as $row)
    {
      ...
      $cats_title = strtolower(str_replace(' ', '-', $st2));
    }
  }
...
<head>
	...
	<meta charset="utf-8">
	<title><?php echo WBName; ?> - <?php echo $category_name; ?></title>
						...
						<div id="navbarSupportedContent" class="collapse navbar-collapse navbar-responsive-collapse">
							<ul class="nav navbar-nav">
								...
								foreach($result as $row)
								{
									...
									?>
									<li <?php if ($row[0] == $category_id) { print '  class="active" ';} ?>>
		...
		<div class="container">
			...
			<div class="row">
				<div class="col-lg-8 col-md-12">
					...
					<div class="block category-listing category-style2 color-red">
						...
						$stmt = $conn->prepare("SELECT * FROM tbl_blog_posts LEFT JOIN tbl_categories ON tbl_blog_posts.category = tbl_categories.id WHERE tbl_blog_posts.visibility = '1' AND tbl_blog_posts.category = ? ORDER BY tbl_blog_posts.id DESC LIMIT $page1,7");
						$stmt->execute([$category_id]);
						$result = $stmt->fetchAll();
          ...
          <div class="paging">
                <ul class="pagination">
                  <?php
                  if (WBCleanURL == "true") {
                  $href = "category/$category_id/$cats_title";
                  }else{
                  $href = "pages/category?key=$category_id";
                  }
                  $add_query = "?";
                  ...
                  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                  $rec = 0;
                  $stmt = $conn->prepare("SELECT * FROM tbl_blog_posts WHERE visibility = '1' AND category = ?");
                  $stmt->execute([$category_id]);
                  $result = $stmt->fetchAll();
                  $rec = count($result);
                  $total_pages = $rec /$rpp;
